...
void move_polar(ILensDataEditorPtr lde, ILDERowPtr row, double r, double phi) {
	...
	cb1_surf->TiltAbout_Y = phi;
	cb1_row->Thickness = r;		// Move to the center of the rowland circle

	ILDERowPtr cb2_row = lde->InsertNewSurfaceAt(row->SurfaceNumber);	// create second coordinate break after input surface
